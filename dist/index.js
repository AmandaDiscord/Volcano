...
const http = http_1.default.createServer(serverHandler);
...
async function serverHandler(req, res) {
    ...
    if (reqPath !== "/" && config.lavalink.server.password && (!req.headers.authorization || req.headers.authorization !== String(config.lavalink.server.password))) {
        Logger_1.default.warn(`Authorization missing for ${req.socket.remoteAddress} on ${req.method.toUpperCase()} ${reqPath}`);
        res.writeHead(401, "Unauthorized", Object.assign({}, Constants_1.default.baseHTTPResponseHeaders, { "Content-Type": "text/plain" })).write("Unauthorized");
        return res.end();
    }
    if (reqPath === "/" && req.method === "GET") {
        res.writeHead(200, "OK", Object.assign({}, Constants_1.default.baseHTTPResponseHeaders, { "Content-Type": "text/plain" })).write("Ok boomer.");
        return res.end();
    }
    if (reqPath === "/loadtracks" && req.method === "GET") {
        ...
        if (!identifier || typeof identifier !== "string")
            return Util_1.default.standardErrorHandler("Invalid or no identifier query string provided.", res, payload, llLog);
        llLog(`Got request to load for identifier "${identifier}"`);
        const match = identifier.match(IDRegex);
        if (!match)
            return Util_1.default.standardErrorHandler("Identifier did not match regex", res, payload, llLog);
        ...
        const doSoundCloudSearch = async () => {
            if ((isSoundcloudSearch || isYouTubeSearch) && !config.lavalink.server.soundcloudSearchEnabled) {
                res.writeHead(200, "OK", Constants_1.default.baseHTTPResponseHeaders).write(JSON.stringify(Object.assign(payload, { loadType: "LOAD_FAILED", exception: { message: "Soundcloud searching is not enabled.", severity: "COMMON" } })));
                res.end();
                return false;
            }
            const data = await (0, soundcloud_1.default)(resource, isSoundcloudSearch || isYouTubeSearch).catch(e => Util_1.default.standardErrorHandler(e, res, payload, llLog));
            if (!data)
                return false;
            ...
            if (tracks.length === 0) {
                Util_1.default.standardErrorHandler("Could not extract SoundCloud info.", res, payload, llLog, "NO_MATCHES");
                return false;
            }
            else {
        ...
        if ((isSoundcloudSearch || (url && url.hostname === soundCloudURL.hostname) || (isYouTubeSearch && canDefaultToSoundCloudSearch))) {
            if (!config.lavalink.server.sources.soundcloud) {
                res.writeHead(200, "OK", Constants_1.default.baseHTTPResponseHeaders).write(JSON.stringify(Object.assign(payload, { loadType: "LOAD_FAILED", exception: { message: "Soundcloud is not enabled.", severity: "COMMON" } })));
                return res.end();
            }
            ...
            if (!config.lavalink.server.sources.local)
                return Util_1.default.standardErrorHandler("Local is not enabled.", res, payload, llLog);
            const data = await (0, local_1.default)(resource).catch(e => Util_1.default.standardErrorHandler(e, res, payload, llLog));
            if (!data)
                return;
            ...
            if (!config.lavalink.server.sources.http)
                return Util_1.default.standardErrorHandler("HTTP is not enabled.", res, payload, llLog);
            const data = await (0, http_2.default)(resource).catch(e => Util_1.default.standardErrorHandler(e, res, payload, llLog));
            if (!data)
                return;
            const info = {
                identifier: resource,
            ...
            try {
                ...
                return Util_1.default.standardErrorHandler(e, res, payload, llLog);
            }
            ...
            if (isYouTubeSearch && canDefaultToSoundCloudSearch)
                await doSoundCloudSearch();
            else {
                if (isYouTubeSearch && !config.lavalink.server.youtubeSearchEnabled) {
                    res.writeHead(200, "OK", Constants_1.default.baseHTTPResponseHeaders).write(JSON.stringify(Object.assign(payload, { loadType: "LOAD_FAILED", exception: { message: "YouTube searching is not enabled.", severity: "COMMON" } })));
                    return res.end();
                }
                if (!config.lavalink.server.sources.youtube) {
                    res.writeHead(200, "OK", Constants_1.default.baseHTTPResponseHeaders).write(JSON.stringify(Object.assign(payload, { loadType: "LOAD_FAILED", exception: { message: "YouTube is not enabled.", severity: "COMMON" } })));
                    return res.end();
                }
                const data = await (0, youtube_1.default)(resource, isYouTubeSearch).catch(e => Util_1.default.standardErrorHandler(e, res, payload, llLog));
                if (!data)
                    return;
                ...
                payload.tracks = tracks;
                if (tracks.length === 0)
                    return Util_1.default.standardErrorHandler("Could not extract YouTube info.", res, payload, llLog, "NO_MATCHES");
                else if (tracks.length === 1 && !data.plData)
                    llLog(`Loaded track ${tracks[0].info.title}`);
            }
        }
        if (payload.tracks.length === 0)
            return Util_1.default.standardErrorHandler("No matches.", res, payload, llLog, "NO_MATCHES");
        else {
            res.writeHead(200, "OK", Constants_1.default.baseHTTPResponseHeaders).write(JSON.stringify(Object.assign({ loadType: payload.tracks.length > 1 && (isYouTubeSearch || isSoundcloudSearch) ? "SEARCH_RESULT" : playlist ? "PLAYLIST_LOADED" : "TRACK_LOADED" }, payload)));
            return res.end();
        }
    }
    if (reqPath === "/decodetracks" && req.method === "GET") {
        ...
        if (!track || !(typeof track === "string" || (Array.isArray(track) && track.every(i => typeof i === "string"))))
            return Util_1.default.standardErrorHandler("Invalid or no track query string provided.", res, {}, llLog);
        let data;
        ...
        res.writeHead(200, "OK", Constants_1.default.baseHTTPResponseHeaders).write(JSON.stringify(data));
        return res.end();
    }
    res.writeHead(404, "Not Found", Constants_1.default.baseHTTPResponseHeaders).write("Not Found");
    return res.end();
}
