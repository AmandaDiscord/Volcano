...
async function install(url: string) {
	if (!url.endsWith("package.json")) return logger.warn("To install a plugin, you must directly link to the package.json file");
	...
	logger.info(`Installed ${fetched.name}@${fetched.version}`);
}
